// game.js - ランダム初期配置 + サウンド（BGM/ボイス/付着音）
//         + STARTでオーディオ解禁 + モバイル操作 + BGM音量UI（Web Audio対応）
//         + 共通発射音は廃止し、アイコン固有ボイスのみ再生
...
(() => {
  ...
  function loadImageSmart(url){
    return new Promise(async (resolve, reject) => {
      ...
      if (hasDotExt) {
        ...
        img.onerror = (e) => resolve(null); // 画像が無くても落とさない
        return;
      }
      try{
  ...
  async function loadAvatars(){
    let data;
    try {
      const resp = await fetch("data/avatars.json", {cache: "no-cache"});
      data = await resp.json();
    } catch (e) {
      console.error("avatars.json の読み込み/パースに失敗:", e);
      data = { palette: ["#1f77b4","#ff7f0e","#2ca02c","#d62728"], avatars: [] };
    }
  ...
  function makeNextBall(){
    ...
    const color = colors.length
      ...
      : palette[Math.floor(Math.random()*palette.length)] || "#1f77b4";
    const pool = avatars.filter(a => (a.color||"").toLowerCase() === (color||"").toLowerCase());
    const avatar = pool.length
      ? pool[Math.floor(Math.random() * pool.length)]
      : avatars[Math.floor(Math.random() * Math.max(avatars.length,1))] || null;
    return avatar ? { color: avatar.color, avatarId: avatar.id } : null;
  }
  ...
  function fire(){
    ...
    if (audioUnlocked) {
      playFireVoice(nextBall.avatarId); // 個別ボイスのみ
    }
